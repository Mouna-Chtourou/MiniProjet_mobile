...
class _FormBuilderState extends State<FormBuilder> {
        ...
        actions: [
          Padding(
            ...
            child: ConstrainedBox(
              constraints: BoxConstraints.tightFor(width: 120, height: 36),
              child: OutlinedButton.icon(
                icon: Icon(Icons.save, color: Color(0xFF048304)),
                label: Text('Save', style: TextStyle(color: Color(0xFF048304))),
                onPressed: () {
            ...
            ElevatedButton(
              onPressed: () {
                if (selectedFieldType != null) {
                  ...
                  showToast(message: "Please select a field type to add.");
                }
              },
  ...
  void saveFormDataToFirestore() async {
    try {
      ...
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => FormListPage()));
    } catch (error) {
      print('Error saving form data to Firestore: $error');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error saving form data: $error'),
